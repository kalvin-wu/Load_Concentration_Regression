...
# Mean Squared Error
test_peak_list = []  # List of average currents

for i in range(0, len(protein_directory_list)):  # outer loop to go through each folder differentiating concentrations
    conc_directory = os.path.join(protein_directory, protein_directory_list[i])  # joins directory path
    conc_directory_list = os.listdir(conc_directory)  # list of files in each concentration folder
    conc_directory_length = len(conc_directory_list)  # Number of files in each folder (these are the excel files)
    os.chdir(conc_directory)  # enter the directory for each concentration folder
    peak_current_list_test = []  # list for peak currents for all files in folder

    # Start file reading loop
    for j in range(0, int(0.5 * conc_directory_length)):  # Loops stop one before the max. Index must start at 0
        data = pd.read_excel(conc_directory_list[4 + j])  # Reading files in the folder. Make sure files are not open.
        ...
        df = df.nlargest(3, 'Current')  # Narrows dataframe down to rows of 3 largest current values
        peak = mean(df['Current'])
        test_peak_list.append(peak)  # 30 files

test_peak_curr = [BSA_avg_peak - x for x in test_peak_list]
pred_conc = [(y - intercept) / slope for y in test_peak_curr]
actual_conc = [3, 3, 3, 3, 3, 0, 0, 0, 0, 0, 4, 4, 4, 4, 4, 1, 1, 1, 1, 1, 5, 5, 5, 5, 5, 2, 2, 2, 2, 2]
diff = []

for i in range(0, len(pred_conc)):
    diff.append(actual_conc[i] - pred_conc[i])

diff_sqr = [y * y for y in diff]
sum_diff2 = sum(diff_sqr)
MSE = sum_diff2 / len(pred_conc)

print("MSE=", MSE)

# Need to write formula for x=f(y)
# 6*5 files = 30 testing points per protein/media
# After each 3 collected, average, then subtract from BSA, then calculate x, take difference, and append to a list
